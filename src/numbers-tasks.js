...
function getAverage(value1, value2) {
  ...
  const remainderValue1 = value1 % 2;
  const remainderValue2 = value2 % 2;

  return halfValue1 + halfValue2 + (remainderValue1 + remainderValue2) / 2;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorModule1 = Math.sqrt(x1 * x1 + y1 * y1);
  const vectorModule2 = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(scalarProduct / (vectorModule2 * vectorModule1));
}
...
function isPrime(n) {
  ...
  for (let counter = 5; counter <= limit; counter += 6) {
...
function getSumOfDigits(num) {
  let sum = 0;
  const numbersArray = Array.from(String(num), Number);
...
function getCountOfOddNumbers(number) {
  ...
  if (absoluteNum === 0) return 0;
  const count = Math.floor(absoluteNum / 2);
  return absoluteNum % 2 === 0 ? count : count + 1;
}
